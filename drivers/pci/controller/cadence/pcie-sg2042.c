 ...
 *   support 32-bit msi address. Basically this is a relatively temporary
 *   solution that may be abandoned in new SoCs in the future.
 ...
 *   compatible with old pcie devices that only support 32-bit msi address. This
 *   would be fixed in new SoC.
...
static void sg2042_pcie_msi_chained_isr(struct irq_desc *desc)
{
	...
	if ((status >> st_msi_in_bit) & 0x1) {
		 ...
		 * processing in irq_ack can not be executed, so we add a
		 * forced clearing operation here.
		 */
		sg2042_pcie_msi_clear_status(pcie);
